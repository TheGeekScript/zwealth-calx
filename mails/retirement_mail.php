...
// Get form data
$email = $_POST['retirement_email'];
$data  = json_decode($_POST['retirement_emailData'], true); // Decode the JSON string
$calcEmailType = $_POST['retirement_calctype']; // Calculator type
...
<table cellspacing="0" cellpadding="10" border="1" style="font-size:11px">
    ...
    <tr>
        <td>Annual Income Required Immediately After Retirement (Rs.)</td>
        <td>'.$data['annualIncomeRequired'].'</td>
    </tr>
    <tr>
        <td>Total Corpus Required for After Retirement (Rs.)</td>
        <td>'.$data['totalCorpusRequired'].'</td>
    </tr>
    <tr>
        <td>Monthly Savings Required to Accumulate the Corpus (Rs.)</td>
        <td>'.$data['monthlySavingsRequired'].'</td>
    </tr>
</table>
';
